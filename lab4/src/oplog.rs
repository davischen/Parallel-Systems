...
impl OpLog {
    ...
    //new
    pub fn append_msg_from_pm(&mut self, msg: &message::ProtocolMessage, t: message::MessageType) {
        self.append(t, msg.txid.clone(), msg.senderid.clone(), msg.opid);
    }
    pub fn append_msg(&mut self, request: &ProtocolMessage) {
        self.append(request.mtype, request.txid.clone(), request.senderid.clone(), request.opid);
    }
